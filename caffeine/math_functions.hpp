...
#define DEFINE_AND_INSTANTIATE_GPU_BINARY_FUNC(name, operation) \
template<typename Dtype> \
__global__ void name##_kernel(const int n, const Dtype* x, const Dtype* y, \
    Dtype* z) { \
  CUDA_KERNEL_LOOP(index, n) { \
    operation; \
  } \
...
template <> \
void caffe_gpu_##name<float>(const int n, const float* x, const float* y, \
    float* z) { \
  /* NOLINT_NEXT_LINE(whitespace/operators) */ \
  name##_kernel<float><<<CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS>>>( \
      n, x, y, z); \
...
template <> \
void caffe_gpu_##name<double>(const int n, const double* x, const double* y, \
    double* z) { \
  /* NOLINT_NEXT_LINE(whitespace/operators) */ \
  name##_kernel<double><<<CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS>>>( \
      n, x, y, z); \
